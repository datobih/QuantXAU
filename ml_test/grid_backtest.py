...
def load_1min(path):
    """Load 1-min OHLC data (no header, tab-separated)."""
...
def build_1min_lookup(df1, df5_index):
    """
    For each 5-min bar timestamp, store its constituent 1-min bars in a dict.
    A 5-min bar at T covers 1-min bars [T, T+1, T+2, T+3, T+4].
    """
    lookup = {}
    end_idx = df1.index[-1]
    for ts in df5_index:
        end_ts = ts + pd.Timedelta(minutes=4, seconds=59)
        if end_ts > end_idx:
            continue
        bars = df1.loc[ts:end_ts]
        if len(bars) > 0:
            lookup[ts] = bars
    return lookup


def same_bar_tp_valid_1min(side, entry, tp, bars_1min):
    """
    Walk 1-min bars in order. Find the first bar that fills the order
    (Low <= entry for BUY, High >= entry for SELL), then check if any
    bar from that point onward — within the same 5-min window — hits TP.
    Returns True only if the fill genuinely preceded the TP.
    """
    fill_bar_idx = None
    rows = list(bars_1min.iterrows())
    for i, (_, b) in enumerate(rows):
        if fill_bar_idx is None:
            if side == 'BUY'  and b['Low']  <= entry:
                fill_bar_idx = i
            elif side == 'SELL' and b['High'] >= entry:
                fill_bar_idx = i
        if fill_bar_idx is not None:
            if side == 'BUY'  and b['High'] >= tp:
                return True
            if side == 'SELL' and b['Low']  <= tp:
                return True
    return False
...
print(f'Building 1-min lookup...')
lookup_1min = build_1min_lookup(df1, df5.index)
print(f'1-min lookup built: {len(lookup_1min):,} 5-min bars covered')
...
for dt, bar in df5.iterrows():
    ...
    bars_1min = lookup_1min.get(dt)   # 1-min bars for this 5-min window
    still_pending = []
    for order in pending_orders:
        if order['side'] == 'BUY' and low <= order['level']:
            if (high >= order['tp'] and bars_1min is not None and
                    same_bar_tp_valid_1min('BUY', order['level'], order['tp'], bars_1min)):
                # 1-min data confirms fill preceded TP within this bar
            ...
            if (low <= order['tp'] and bars_1min is not None and
                    same_bar_tp_valid_1min('SELL', order['level'], order['tp'], bars_1min)):
    ...
    # Skip positions filled THIS bar — they were put into open_positions because
    # the same-bar TP check in Step 3 deemed it invalid (e.g. open >= tp on a
    # crash bar). Checking them again here with the same high/low would TP them
    # anyway, defeating the guard. They'll be checked on the next bar.
    still_open = []
    for pos in open_positions:
        if pos.open_time == dt:
